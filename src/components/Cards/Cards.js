...
const Cards = ({ results }) => {
  ...
  if (results) {
    display = results.map( x => {
      ...
      return( 
      <div key={id} className="col-12 col-md-6 col-lg-4 mb-4 d-flex position-relative">
        <div className= {`${styles.cards} h-100 w-100`}>
          <img src={image} alt="" className={`${styles.img} img-fluid w-100`}/>
    ...
    display = <h1 className='text-center'>No characters Found :/</h1>
  }
  return <>{display}</>
}
